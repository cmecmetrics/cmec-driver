	...
	for key in dirList: 
		if isinstance(dirList[key],str) and len(dirList[key]) == 0:
			...
			if tmpdir.absolute().is_dir():
				...
				raise CMECError(tmpdir.absolute() + " does not exist or is not a directory")
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description = "Process command line cmec-driver input")
	...
	if args.command == "register":
		if args.module:
			cmec_register(args.module[0])
		else:
			print("Usage: python cmec-driver.py register -module <module directory>")
	...
	if args.command == "unregister":
		if args.module:
			...
			print("Usage: python cmec-driver.py unregister -module <module directory>")
	...
	if args.command == "run":
		if (args.obs and args.model and args.output and args.module):
			...
			print("Usage: python cmec-driver.py run -obs <observation directory> -model <model directory> -output <output directory> -module <module name>")
